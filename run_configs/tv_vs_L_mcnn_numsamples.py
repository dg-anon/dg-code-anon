...
def sgd_const():
    ...
    search.add_list("numsamples", [100, 250]) #[500, 1000, 2000, 5000, 10000, 20000]
    search.add_static_var("batchsize", 128)
    search.add_static_var("testbatchsize", 128)

    search.add_static_var("noise_probability", 0.3)
    search.add_list("noise_method", ['fl_cifar_spdc_cm_pcflip', 'fl_cifar_spdc_cm_fine_check'])
    search.add_static_var("fine_labels", '-')
    ...
    search.add_list("losstype", ['ce'])
    
    search.add_static_var("total_iterations", 100000)
    search.add_static_var("tr_loss_thresh", 1e-6)
    search.add_static_var("lr_sched_type", 'const')
    search.add_list("lr", [1.0])

    search.add_static_var("eval_iter", 100)
    ...
    search.add_static_var("wandb_project_name", 'de-tv-v-L-numsamples-mcnn-cifar-spdc')
