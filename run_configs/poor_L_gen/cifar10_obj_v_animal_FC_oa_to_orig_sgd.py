...
def sgd_const():
    ...
    search.add_list("noise_method", ['fl_cifar_OA_cm_oa_orig', 'fl_cifar_OA_cm_fine_check'])
    ...
    search.add_static_var("classifiername", 'FC_relu')
    search.add_static_var("numinput", 3072)
    search.add_static_var("numhid", 512)
    search.add_static_var("depth", 4)
    ...
    search.add_static_var("lr_sched_type", 'decay')
    search.add_list("lr", [0.01])
    search.add_static_var("momentum", 0.9)
    search.add_static_var("lr_decay", 0.99)
    
    search.add_static_var("total_iterations", 320000)
    search.add_static_var("tr_loss_thresh", 1e-6)

    search.add_static_var("wandb_project_name", 'poor-L-cifar10-oa-to-orig')    
